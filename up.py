...
# состав 320
airbus_320 = {
    1: "Носовое колесо весом 50 кг. и габаритами 740х230.",
...
def correct_flyKit(fkt):
    ...
    ans = result.split()
    ans_after = "".join(ans)
    if ans_after == "носовоеколесо":
        return 1
    if ans_after == "основноеколесо":
        return 2
    if ans_after == "колесашасси":
        return 3
    if ans_after == "колесашасси+10m+5h" or ans_after == "колесашасси+10+5":
        return 4
    if ans_after == "полнаякомплектация+домкрат+и\я+hst":
        return 5
    if ans_after == "полнаякомплектация+домкрат+и\я+hst+10m+5h" or ans_after == "полнаякомплектация+домкрат+и\я+hst+10+5":
        return 6
    if ans_after == "полнаякомплектация+домкрат+и\я+hst+20m+5h" or ans_after == "полнаякомплектация+домкрат+и\я+hst+20+5":
        return 7
    if ans_after == "полнаякомплектация+домкрат+и\я+hst+19m+5h" or ans_after == "полнаякомплектация+домкрат+и\я+hst+19+5":
        return 8
...
def structure_FKT(lst):
    ...
    for i in range(len(lst)):
        flight, type_Air, point, fly_kit, time = lst[i][0], str(lst[i][5]).strip(), lst[i][3].strip(), lst[i][4], lst[i][2]
        text, long, air = "", [], ""

        if correct_air(type_Air) == 737:
            if correct_flyKit(fly_kit) in boeing:
                text = boeing[correct_flyKit(fly_kit)]
                long = [i for i in text.split("\n")]
                air = "B737"
        if correct_air(type_Air) == 320:
            if correct_flyKit(fly_kit) in airbus_320:
                text = boeing[correct_flyKit(fly_kit)]
                long = [i for i in text.split("\n")]
                air = "A320"
        if correct_air(type_Air) == 321:
            if correct_flyKit(fly_kit) in airbus_321:
                text = boeing[correct_flyKit(fly_kit)]
                long = [i for i in text.split("\n")]
                air = "A321"

        message = f"{date_plus}  {flight}  SVO-{point}, вылет {time} (МСК), BC ({air}), необходимо загрузить и закрепить в БГО ВС {len(long)} {correct_long(len(long))} технической аптечки: \n{text}"
...
root = tk.Tk()
root.title("Загрузка тех-аптечек")

# Приветствие
greeting_label = tk.Label(root, text=f"Добро пожаловать в программу для создания письма на вылет для Москва Карго!\n Программа понимает:\nносовое колесо\nосновное колесо\nколесашасси\nколесашасси+10m+5h\nколесашасси+10+5\nполнаякомплектация+домкрат+и\я+hst\nполнаякомплектация+домкрат+и\я+hst+10m+5h\nполнаякомплектация+домкрат+и\я+hst+20m+5h", font=("Arial", 12))
